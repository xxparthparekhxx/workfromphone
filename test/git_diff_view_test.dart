...
''';
  ...
  test('GitDiffParser parses hunk headers, old line numbers, and new line numbers correctly', () {
    ...
    final addition1 = lines.firstWhere((l) => l.type == DiffLineType.addition && l.text.contains('New message'));
    expect(addition1.oldLineNum, isNull);
    expect(addition1.newLineNum, 11);

    final addition2 = lines.firstWhere((l) => l.type == DiffLineType.addition && l.text.contains('Added line'));
    expect(addition2.oldLineNum, isNull);
    expect(addition2.newLineNum, 12);
  });

  testWidgets('GitDiffView renders diff line numbers and styled text', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: GitDiffView(rawDiff: sampleDiff),
        ),
      ),
    );
